...
/**
 * DELETE /api/ip/delete?ip=1.2.3.4
 * Proxies to Taxii 2.x: POST /ip/delete with body { "ip": "1.2.3.4" }.
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${base}/ip/delete`, {
      method: 'POST',
      headers: getHeaders(),
    ...
    return NextResponse.json({ message: 'Removed', ip });
  } catch (e) {
    console.error('IP delete error:', e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Failed to delete IP' },
      { status: 500 }
    );
  }
}
